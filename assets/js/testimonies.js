(function () {
	...
	var change_testimoniy = function(new_id) {
	...
	document.querySelector(".left").addEventListener("click", function() {
		change_testimoniy(active_id - 1 + testimonies_count * (active_id == 1));
	});
	
	document.querySelector(".right").addEventListener("click", function() {
		change_testimoniy(active_id + 1 - testimonies_count * (active_id == testimonies_count));
	})

	for (var i = 1; i <= testimonies_count; i++) {
		(function(id) {
			document.querySelector("#button-"+id+"-testimonies").addEventListener("click", function () {
				change_testimoniy(id);
			});
		})(i);
	}
})();
